...
export default function MarketingBlastPage() {
  ...
  const [templateKey, setTemplateKey] = useState<string>("tickets-live");
  ...
  function resetAfterSend() {
    setResult(null); setSubject(""); setHeadline(""); setBody(""); setEventId(""); setTemplateKey("tickets-live");
    setSelTags([]); setSelBatches([]); setSelCities([]); setSelStates([]); setSelAttended([]); setSource("");
    setPreview(null); setTestMsg(null);
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      <div>
        ...
        <p className="font-ui text-ink-muted text-sm mt-0.5">Pick a list, choose an event, select a ready-made template — then preview, test, and send. Honors marketing opt-outs.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5 items-start">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-ivory-200 p-5">
            <StepHead n={1} title="Who to reach" sub="Filter your community into a list" done={!!hasFilters} />
          ...
          {/* Step 2 — Event */}
          <div className="bg-white rounded-2xl border border-ivory-200 p-5">
            <StepHead n={2} title="Choose an event" sub={tmpl?.requiresEvent ? "Required for this template" : "Optional"} done={!!eventId} />
            ...
            <div className="max-h-64 overflow-auto rounded-xl border border-ivory-200 divide-y divide-ivory-200">
              {!tmpl?.requiresEvent && (
          ...
          {/* Step 3 — Template */}
          <div className="bg-white rounded-2xl border border-ivory-200 p-5">
            <StepHead n={3} title="Pick a template" sub="Designed for you — no copywriting needed" done={contentReady} />
                ...
                return (
                  <button key={t.key} onClick={() => setTemplateKey(t.key)} className={`w-full flex items-start gap-3 px-3.5 py-3 rounded-xl border text-left transition-all ${on ? "border-aubergine bg-aubergine/[0.04] ring-1 ring-aubergine/20" : "border-ivory-200 hover:border-aubergine/30"}`}>
                    ...
                    <span className="min-w-0">
                      <span className="font-display font-bold text-ink text-sm block">{t.name}</span>
        ...
        <div className="space-y-4 lg:sticky lg:top-4">
          ...
          <div className="bg-white rounded-2xl border border-ivory-200 p-5 space-y-4">
            <p className="font-display font-bold text-ink text-sm">Review &amp; send</p>
            ...
            <div className="rounded-xl border border-ivory-200 bg-ivory/40 divide-y divide-ivory-200">
              {[
                ...
                ["Event", eventId ? eventTitleOf(eventId) : "— none"],
      ...
      {confirming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => !sending && setConfirming(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="rounded-xl border border-ivory-200 bg-ivory/40 divide-y divide-ivory-200">
              {[["Recipients", String(count ?? 0)], ["Template", tmpl ? tmpl.name : "—"], ["Event", eventId ? eventTitleOf(eventId) : "— none"]].map(([k, v]) => (
