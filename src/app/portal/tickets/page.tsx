    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-4 flex items-start justify-between gap-3 border-b border-ivory-200">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-widest text-ink-muted">Transfer Ticket{selectedSeats.length !== 1 ? "s" : ""}</p>
            <p className="font-display font-bold text-ink text-lg mt-0.5" style={{ letterSpacing: "-0.015em" }}>{order.eventTitle}</p>
            <p className="font-ui text-xs text-ink-muted">{order.tierName}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-ivory flex items-center justify-center text-ink-muted hover:text-ink transition-colors shrink-0">
          ...
          {step === "seats" && (
            <div className="space-y-4">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left ${
                          ...
                          isSelected ? "bg-aubergine/8 border-aubergine/30" : "bg-white border-ivory-200 hover:border-aubergine/20"
                        }`}
                      >
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-2xl font-display font-bold text-sm transition-all ${seatsValid ? "bg-aubergine text-white hover:bg-aubergine-light active:scale-[0.98]" : "bg-ivory text-ink-muted/50 cursor-not-allowed"}`}
              >
                {seatsValid ? `Transfer ${selectedSeats.length} ticket${selectedSeats.length !== 1 ? "s" : ""} →` : "Select at least one ticket"}
              </button>
            </div>
          )}
          ...
          {step === "confirm" && (
            <div className="space-y-4">
              <div className="rounded-2xl bg-ivory border border-ivory-200 p-4 space-y-3">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    { label: "Seats", value: selectedSeats.length === order.qty ? `All ${order.qty} tickets` : selectedSeats.map(s => `T${s}`).join(", ") },
              ...
              <div className="flex gap-2">
                <button onClick={() => setStep("email")} className="flex-1 py-3 rounded-2xl border border-ivory-200 font-ui text-sm font-semibold text-ink-muted hover:text-ink transition-colors">← Back</button>
                <button disabled={submitting} onClick={handleConfirm} className="flex-1 py-3 rounded-2xl bg-aubergine text-white font-display font-bold text-sm hover:bg-aubergine-light active:scale-[0.98] transition-all flex items-center justify-center gap-2">
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden transition-all ${
      ...
      <div className="flex items-center">
        <button onClick={() => !isPending && !isTransferred && setExpanded(!expanded)} className={`flex-1 text-left px-4 py-3.5 flex items-center gap-3 ${isPending || isTransferred ? "cursor-default" : ""}`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <p className={`font-display font-bold text-sm ${isTransferred ? "text-ink-muted" : "text-ink"}`}>{ticketId}</p>
              <span className={`font-mono text-[9px] uppercase tracking-wide px-2 py-0.5 rounded-full border font-bold ${
                ...
                {isTransferred ? "Transferred" : isPending ? "Pending" : "In Wallet"}
              </span>
            </div>
            <p className={`font-ui text-xs ${isTransferred || isPending ? "text-ink-muted/60" : "text-ink-muted"}`}>
              {isPending || isTransferred
                ? `→ ${transfer!.toName || transfer!.toEmail}`
                : `${tierName} · Seat ${seat} of ${total}`}
            </p>
          </div>
        </button>

        {/* Per-seat transfer button — only on free upcoming seats */}
        {!isPending && !isTransferred && onTransfer && (
          <button
            onClick={() => onTransfer(seat)}
            className="mr-3 shrink-0 flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-aubergine/20 text-aubergine font-ui font-semibold text-[11px] hover:bg-aubergine/5 transition-all"
          >
        ...
        {!isPending && !isTransferred && (
          <button onClick={() => setExpanded(!expanded)} className="pr-4">
        ...
        <>
          ...
          <div className="px-4 pb-4 pt-3">
            <div className="flex flex-col sm:flex-row gap-4 items-start">
              <div className="flex flex-col items-center gap-2">
                <div className="p-3 rounded-2xl border-2 border-marigold/20 bg-white shadow-sm"><QRCode value={ticketId} size={150} /></div>
              ...
              <div className="flex-1">
                <p className="font-mono text-[10px] uppercase tracking-widest text-ink-muted mb-2">Ticket Details</p>
                {[{ label: "Event", value: eventTitle }, { label: "Type", value: tierName }, { label: "Ticket ID", value: ticketId }].map(row => (
                  <div key={row.label} className="flex gap-2 py-1.5 border-b border-ivory-200 last:border-0">
                    <span className="font-ui text-xs text-ink-muted w-16 shrink-0">{row.label}</span>
                    <span className="font-ui text-xs font-semibold text-ink">{row.value}</span>
                  </div>
  ...
  return (
    <>
      ...
      <div className={`rounded-2xl overflow-hidden border transition-all ${
        ...
        <button onClick={() => setExpanded(!expanded)} className="w-full text-left">
          <div className="flex items-stretch">
            <div className="w-1.5 shrink-0 rounded-l-2xl" style={{ backgroundColor: allTransferred ? "#B0B0C0" : artistColor }} />
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {order.groupId && <span className="font-mono text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-full font-bold bg-marigold/15 text-marigold-dark">Group Order</span>}
                  </div>
                  <h3 className={`font-display font-bold leading-snug mb-0.5 ${allTransferred ? "text-ink-muted" : "text-ink"}`}>{order.eventTitle}</h3>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2">
                    ...
                    {order.venue && <span className="font-ui text-xs text-ink-muted">📍 {order.venue}</span>}
                    <span className="font-ui text-xs text-ink-muted">
                      🎟️ {order.qty} × {order.tierName}
                      {hasAnyTransfer && (
                        <span className="ml-1 text-ink-muted/60">
                          · {freeCount > 0 ? `${freeCount} in wallet` : ""}
                          {pendingCount > 0 ? `${freeCount > 0 ? ", " : ""}${pendingCount} pending` : ""}
                          {acceptedCount > 0 ? `${freeCount > 0 || pendingCount > 0 ? ", " : ""}${acceptedCount} transferred` : ""}
                        </span>
                      )}
                    </span>
                  </div>
                ...
                <div className="flex flex-col items-center gap-1 border-l border-ivory-200 pl-4 shrink-0">
                  {daysUntil !== null && !allTransferred ? (
                    <><p className="font-display font-bold text-aubergine text-2xl leading-none">{daysUntil}</p><p className="font-mono text-[9px] uppercase tracking-wide text-ink-muted">days</p></>
                  ) : (
                    <p className="font-mono text-[10px] text-ink-muted">{allTransferred ? "Gone" : "Past"}</p>
                  )}
                  <svg className={`w-4 h-4 text-ink-muted mt-1 transition-transform duration-200 ${expanded ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                </div>
              </div>
            </div>
          </div>
