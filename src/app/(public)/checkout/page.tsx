...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    // Check auth session and auto-populate
  ...
  const paymentValid = cardNumber.replace(/\s/g, "").length === 16 && expiry.length === 5 && cvv.length >= 3 && nameOnCard && billingZip.length === 5;
  ...
  async function handlePurchase(e: React.FormEvent) {
    ...
    if (orderId && payload.groupId && payload.groupEmail) {
      await markMemberPaid({
        groupId: payload.groupId,
        email: payload.groupEmail,
        orderId,
      });
    }

    if (error) {
      // Non-fatal — order may have saved partially; continue to confirmation
      console.error("Order save error:", error);
    }

    // Save minimal order to localStorage for confirmation page
    const orderSnapshot = {
            ...
            {step === "contact" ? (
              ...
              <form onSubmit={handlePurchase} className="space-y-5">
                ...
                <div className="rounded-xl bg-marigold/8 border border-marigold/20 px-4 py-3">
                  ...
                  <p className="font-ui text-xs text-ink-muted">Use card <span className="font-mono font-bold text-ink">4242 4242 4242 4242</span>, any future date, any CVV.</p>
                </div>
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl bg-peacock/5 border border-peacock/20">
                  ...
                  <p className="font-ui text-xs text-ink-muted">Your payment info is encrypted with 256-bit SSL. We never store your card details.</p>
                </div>
                ...
                {payload && (
                  <div className="border-t border-ivory-200 pt-3 space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-ui text-ink">${(payload.unitPrice * payload.qty).toLocaleString()}</span>
                    </div>
                    {payload.discount > 0 && (
                      <div className="flex justify-between text-xs">
                        <span className="font-ui text-peacock">Group discount ({payload.discount}%)</span>
                        <span className="font-ui text-peacock">−${payload.discountAmount.toLocaleString()}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-xs">
                      <span className="font-ui text-ink-muted">Service fee</span>
                      <span className="font-ui text-ink-muted">${payload.serviceFee.toLocaleString()}</span>
                    </div>
                ...
                {payload && payload.discount > 0 && (
                  <div className="rounded-xl bg-marigold/10 border border-marigold/25 p-3">
                    <p className="font-ui text-xs font-semibold text-marigold-dark text-center">
                      Saving ${payload.discountAmount.toLocaleString()} with group pricing!
                    </p>
                  </div>
                )}
